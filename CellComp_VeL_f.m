function [Ve_L_f] = CellComp_VeL_f(Xi, Yi, Zi, I_M, I_D, z_max, t_max, d_z, d_t)
%%CELLCOMP_VEL_F
% CELLCOMP_VEL_F returns the longitudinal component of the
...
% OUTPUTS:
%
% Ve_L_f                The calculated extracellular potential along the
...
for j=1:length(w)
    ...
    for i = 1:length(I_M)
        ...
        Ve_L_f_sources(i,:,j) = I_hat.*exp(-1i*kz_m*Zi(i))./(xi_T_f*(2*pi)^(3/2)) ...
