function [Ve_L_f,Ve_T_X_f,Ve_T_Y_f,Ve_T_Z_f,neur_eq,kzp_m] = CellComp4Layer_Ve_f_Plane( ...
    Xi, Yi, Zi, I_M, I_D, ...
    x_max, z_max, t_max, d_x, d_z, d_t, ...
    h_F, Ya, rot, Ri, ...
    type, iso, biphasic)
...
for j = 1:length(w)
    ...
    eta_L = sqrt((kx_m.^2*sigma_L + kz_m.^2*sigma_L)./sigma_L);
    ...
    for i = 1:length(I_M)
        ...
        coeff_denom = exp(2.*Yi(i).*eta_V).*exp(2.*eta_F.*h_F).*(eta_V.*sigma_V + eta_F.*xi_T_f).* ...
            (eta_L.*sigma_L + eta_F.*xi_T_f) - exp(2.*eta_F.*h_F).* ...
            (eta_V.*sigma_V - eta_F.*xi_T_f).*(eta_L.*sigma_L + eta_F.*xi_T_f) - ...
            eta_F.^2.*xi_T_f.^2.*(exp(2.*Yi(i).*eta_V) + 1) - eta_L.*eta_V.*sigma_V.*sigma_L.* ...
            (exp(2.*Yi(i).*eta_V) - 1) + eta_F.*eta_V.*sigma_V.*xi_T_f.*(exp(2.*Yi(i).*eta_V) - 1) + ...
            eta_F.*eta_L.*sigma_L.*xi_T_f.*(exp(2.*Yi(i).*eta_V) + 1);
        
        % Vitreous
        if Ya > 0 && Ya <= Yi(i)
            
            B_1 = (m.*exp(Yi(i).*eta_V + 2.*eta_F.*h_F).*(eta_V.*sigma_V - ...
                eta_F.*xi_T_f).*(eta_L.*sigma_L + eta_F.*xi_T_f) - ...
                m.*exp(Yi(i).*eta_V).*(eta_V.*sigma_V + eta_F.*xi_T_f).*(eta_L.*sigma_L - ...
                eta_F.*xi_T_f))./(eta_V)./coeff_denom;
            
            B_2 = (eta_V.*m.*sigma_V.*cosh(Yi(i).*eta_V).*(eta_F.*xi_T_f - ...
                eta_L.*sigma_L + eta_L.*sigma_L.*exp(2.*eta_F.*h_F) + ...
                eta_F.*xi_T_f.*exp(2.*eta_F.*h_F)) - eta_F.^2.*m.*xi_T_f.^2.*sinh(Yi(i).*eta_V) ...
                + eta_F.*eta_L.*m.*sigma_L.*xi_T_f.*sinh(Yi(i).*eta_V) + ...
                eta_F.*m.*xi_T_f.*exp(2.*eta_F.*h_F).*sinh(Yi(i).*eta_V).*(eta_L.*sigma_L + ...
                eta_F.*xi_T_f))./(eta_V)./coeff_denom;
        ...
        elseif Ya <= 0 && Ya >= -h_F
            ...
            C_1 = -(2.*m.*sigma_V.*exp(Yi(i).*eta_V).*(eta_L.*sigma_L - ...
                eta_F.*xi_T_f))./coeff_denom;
            
            C_2 = (2.*m.*sigma_V.*exp(Yi(i).*eta_V).*exp(2.*eta_F.*h_F).*(eta_L.*sigma_L + ...
                eta_F.*xi_T_f))./coeff_denom;
