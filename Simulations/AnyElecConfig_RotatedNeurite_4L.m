...
%% Plots of membrane potential at end of cathodic phase

ColorSet = varycolor(length(rot)+1);
ColorSet = [[1 .5 0]; ColorSet];

% % Longitudinal
% P1 = figure('outerposition',[scrsz(2,3)/2+1 31 scrsz(2,3)/2 scrsz(2,4)-31],'Color',[1 1 1]);
% ax = subplot(3,1,1);
% set(ax, 'ColorOrder', ColorSet);
% hold on
% plot(Z*1e6,squeeze(Vm_L_rot_AOP(1,:,floor((t_max+I_D(1))/d_t)))*1e3)
% for i = 1:length(rot)
%     plot(Z*1e6,squeeze(Vm_L_rot(i,:,floor((t_max+I_D(1))/d_t)))*1e3)
% end
% title({'Membrane activation at end of cathodic phase for various neurite orientations';...
%     ' ';'V_{m,L}'})
% ylabel('V_{m,L} (mV)')
% 
% % Transverse
...
% set(ax, 'ColorOrder', ColorSet);
% hold on
% plot(Z*1e6,squeeze(Vm_T_rot_AOP(1,:,floor((t_max+I_D(1))/d_t)))*1e3)
% for i = 1:length(rot)
%     plot(Z*1e6,squeeze(Vm_T_rot(i,:,floor((t_max+I_D(1))/d_t)))*1e3)
% end
% title('V_{m,T}')
% ylabel('V_{m,T} (mV)')
% 
...
% set(ax, 'ColorOrder', ColorSet);
% hold on 
% plot(Z*1e6,squeeze(Vm_rot_AOP(1,:,floor((t_max+I_D(1))/d_t)))*1e3)
% for i = 1:length(rot)
%     plot(Z*1e6,squeeze(Vm_rot(i,:,floor((t_max+I_D(1))/d_t)))*1e3)
% end
% title('V_{m}')
% xlabel('Neurite axis (\mum)')
% ylabel('V_{m} (mV)')
% 
% set(P1, 'Colormap', ColorSet(1:end-1,:));
% cb = colorbar('East');
% caxis([0 110])
% set(cb,'Ticks',([rot rot(end)+d_rot]+d_rot/2)*180/pi,'TickLabels',['AOP' num2cell(rot*180/pi)],'YAxisLocation','right')
% ylabel(cb,'Neurite rotation')
% 
% %% Plots of maximum membrane potential at each angle
% 
% % Longitudinal
% P5 = figure('outerposition',[scrsz(1,1) 31 scrsz(1,3)/2 scrsz(1,4)-31],'Color',[1 1 1]);
% ax = subplot(3,1,1);
% colormap(ax,ColorSet(2:end-1,:))
% p = cline(rot*180/pi,max(abs(Vm_L_rot(:,floor((z_max)/d_z+1),:))*1e3,[],3));
% set(p,'LineWidth',2)
% title({'Maximum membrane activation for various neurite orientations';...
%     ' ';'V_{m,L}^{max}'})
% ylabel('V_{m,L}^{max} (mV)')
% box off
% 
% % Transverse
% ax = subplot(3,1,2);
% colormap(ax,ColorSet(2:end-1,:))
% p = cline(rot*180/pi,max(abs(Vm_T_rot(:,floor((z_max)/d_z+1),:))*1e3,[],3));
% set(p,'LineWidth',2)
% title('V_{m,T}^{max}')
% ylabel('V_{m,T}^{max} (mV)')
% box off
% 
% % Transverse
% ax = subplot(3,1,3);
% colormap(ax,ColorSet(2:end-1,:))
% p = cline(rot*180/pi,max(abs(Vm_rot(:,floor((z_max)/d_z+1),:))*1e3,[],3));
% set(p,'LineWidth',2)
% title('V_{m}^{max}')
% xlabel('Angle of neurite from z-axis (degrees)')
% ylabel('V_{m}^{max} (mV)')
% box off
% 
...
% 
% % Longitudinal
% P2 = figure('outerposition',[scrsz(1,1)+scrsz(1,3)/2 31 scrsz(1,3)/2 scrsz(1,4)-31],'Color',[1 1 1]);
% ax = subplot(3,1,1);
% hold on
% set(ax, 'ColorOrder', ColorSet);
